...
__host__ __device__ inline std::uint64_t compact_index(int x, int y, int z, int nx, int ny) {
    return static_cast<std::uint64_t>(z) * static_cast<std::uint64_t>(ny) * static_cast<std::uint64_t>(nx) +
           static_cast<std::uint64_t>(y) * static_cast<std::uint64_t>(nx) + static_cast<std::uint64_t>(x);
}
...
__global__ void copy_compact_to_ghosted_kernel(
    float* ghosted,
    const float* compact,
    int nx,
    int ny,
...
__global__ void copy_ghosted_to_compact_kernel(
    float* compact,
    const float* ghosted,
    int nx,
    int ny,
...
__global__ void splat_density_compact_kernel(
    float* density,
    float center_x,
    float center_y,
    float center_z,
...
__global__ void splat_force_compact_kernel(
    float* u,
    float* v,
    float* w,
    float center_x,
...
__global__ void rbgs_diffuse_kernel(
    float* dst,
    const float* src,
    float alpha,
    float denom,
    ...
    const float neighbors =
        dst[ghosted_index(i - 1, j, k, nx_total, ny_total)] + dst[ghosted_index(i + 1, j, k, nx_total, ny_total)] +
        dst[ghosted_index(i, j - 1, k, nx_total, ny_total)] + dst[ghosted_index(i, j + 1, k, nx_total, ny_total)] +
        dst[ghosted_index(i, j, k - 1, nx_total, ny_total)] + dst[ghosted_index(i, j, k + 1, nx_total, ny_total)];
...
__global__ void divergence_kernel(
    ...
    const int idx = ghosted_index(i, j, k, nx_total, ny_total);
    divergence[idx] = half_inv_h *
        ((u[ghosted_index(i + 1, j, k, nx_total, ny_total)] - u[ghosted_index(i - 1, j, k, nx_total, ny_total)]) +
         (v[ghosted_index(i, j + 1, k, nx_total, ny_total)] - v[ghosted_index(i, j - 1, k, nx_total, ny_total)]) +
         (w[ghosted_index(i, j, k + 1, nx_total, ny_total)] - w[ghosted_index(i, j, k - 1, nx_total, ny_total)]));
}
...
__global__ void subtract_gradient_kernel(
    ...
    const int idx = ghosted_index(i, j, k, nx_total, ny_total);
    u[idx] -= half_inv_h * (pressure[ghosted_index(i + 1, j, k, nx_total, ny_total)] -
                            pressure[ghosted_index(i - 1, j, k, nx_total, ny_total)]);
    v[idx] -= half_inv_h * (pressure[ghosted_index(i, j + 1, k, nx_total, ny_total)] -
                            pressure[ghosted_index(i, j - 1, k, nx_total, ny_total)]);
    w[idx] -= half_inv_h * (pressure[ghosted_index(i, j, k + 1, nx_total, ny_total)] -
                            pressure[ghosted_index(i, j, k - 1, nx_total, ny_total)]);
}

__global__ void pcg_initialize_kernel(
    float* pressure,
    float* r,
    float* z,
    float* p,
...
__global__ void pcg_apply_preconditioner_kernel(
    float* z,
    const float* r,
    float diag_inv,
    int nx,
...
__global__ void pressure_matvec_kernel(
    float* ap,
    const float* p,
    float inv_h2,
    int nx,
    ...
    const float neighbors = inv_h2 *
        (p[ghosted_index(i - 1, j, k, nx_total, ny_total)] + p[ghosted_index(i + 1, j, k, nx_total, ny_total)] +
         p[ghosted_index(i, j - 1, k, nx_total, ny_total)] + p[ghosted_index(i, j + 1, k, nx_total, ny_total)] +
         p[ghosted_index(i, j, k - 1, nx_total, ny_total)] + p[ghosted_index(i, j, k + 1, nx_total, ny_total)]);
...
__global__ void residual_update_kernel(
    float* r,
    const float* ap,
    float alpha,
    int nx,
...
__global__ void direction_update_kernel(
    float* p,
    const float* z,
    float beta,
    int nx,
...
Solver3D::Solver3D(const SolverDesc& desc) : desc_(desc) {
    ...
    auto allocate = [this](float*& field) {
        check_cuda(cudaMalloc(reinterpret_cast<void**>(&field), ghosted_bytes_), "cudaMalloc ghosted field");
    };
...
void Solver3D::copy_compact_to_ghosted_(float* ghosted, const float* compact) {
    constexpr int block_size = 256;
    const int grid_size = static_cast<int>((interior_cell_count_ + block_size - 1) / block_size);
    copy_compact_to_ghosted_kernel<<<grid_size, block_size>>>(
        ghosted,
        compact,
        desc_.nx,
        desc_.ny,
...
void Solver3D::copy_ghosted_to_compact_(float* compact, const float* ghosted) {
    constexpr int block_size = 256;
    const int grid_size = static_cast<int>((interior_cell_count_ + block_size - 1) / block_size);
    copy_ghosted_to_compact_kernel<<<grid_size, block_size>>>(
        compact,
        ghosted,
        desc_.nx,
        desc_.ny,
...
void Solver3D::add_density_sphere(const FieldSet& fields, float x, float y, float z, float radius, float amount) {
    ...
    const dim3 block = make_block(desc_);
    const dim3 grid = make_grid(desc_.nx, desc_.ny, desc_.nz, block);
    splat_density_compact_kernel<<<grid, block>>>(
        reinterpret_cast<float*>(fields.density.data),
        x,
        y,
        z,
        fmaxf(radius, 1.0f),
...
void Solver3D::add_force_sphere(const FieldSet& fields, float x, float y, float z, float radius, float fx, float fy, float fz) {
    ...
    const dim3 block = make_block(desc_);
    const dim3 grid = make_grid(desc_.nx, desc_.ny, desc_.nz, block);
    splat_force_compact_kernel<<<grid, block>>>(
        reinterpret_cast<float*>(fields.velocity_x.data),
        reinterpret_cast<float*>(fields.velocity_y.data),
        reinterpret_cast<float*>(fields.velocity_z.data),
        x,
        y,
        z,
        fmaxf(radius, 1.0f),
...
void Solver3D::advect_velocity_(
    float* u_dst,
    float* v_dst,
    float* w_dst,
    const float* u_src,
    ...
    const dim3 grid = make_grid(desc_.nx, desc_.ny, desc_.nz, block);
    const float dt_over_h = desc_.dt / desc_.cell_size;
    advect_velocity_kernel<<<grid, block>>>(
        u_dst,
        v_dst,
        w_dst,
        u_src,
...
void Solver3D::project_(float* u, float* v, float* w) {
    ...
    pcg_initialize_kernel<<<grid, block>>>(
        pressure_,
        pcg_r_,
        pcg_z_,
        pcg_p_,
    ...
    for (int iteration = 0; iteration < desc_.pressure_iterations; ++iteration) {
        ...
        pcg_apply_preconditioner_kernel<<<grid, block>>>(
            pcg_z_,
            pcg_r_,
            diag_inv,
            desc_.nx,
...
int32_t stable_fluids_fields_add_density_splat(
    ...
    return stable_fluids_try(context, [&] {
        context->solver->add_density_sphere(*fields, splat->center_x, splat->center_y, splat->center_z, splat->radius, splat->amount);
    });
}

int32_t stable_fluids_fields_add_force_splat(
    StableFluidsContext* context,
    const StableFluidsFieldSetDesc* fields,
    ...
    return stable_fluids_try(context, [&] {
        context->solver->add_force_sphere(
            *fields,
            splat->center_x,
            splat->center_y,
            splat->center_z,
